...
jQuery(document).ready(loadjplayer());

function loadjplayer() {
	if (0 < $(".sb-xiami").length) {
		...
		r = ajax.swf;
		$(".sb-xiami").each(function() {
			var a = $(this),
			c = a.children(".sb-player").find(".sb-cover"),
			t = a.children(".sb-player").children('.sb-info').find(".sb-title"),
			d = a.attr("songid");
			$.ajax({
				type: "get",
				dataType: "jsonp",
				jsonp: "callback",
				url: "http://nuaaded.sinaapp.com/xiami",
				data: {
					id: d,
				},
				async: !1,
				...
				success: function(b) {
					if (b) {
						c.append('<img src="' + b.pic + '" />');
						t.append(b.title);
						var d = a.children(".sb-jplayer");
						d.jPlayer({
							ready: function() {
								d.jPlayer("setMedia", {
									mp3: b.location
		...
		$(".sb-xiami .play-button").click(function() {
			var a = $(this).parent().parent().children(".sb-jplayer"),
			c = a.attr("id");
			$(".sb-xiami").each(function() {
				var a = $(this).children(".sb-jplayer");
				a.attr("id") != c && a.jPlayer("pause")
			});
			a.data().jPlayer.status.paused ? a.jPlayer("play") : a.jPlayer("pause")
		});
		$(".sb-xiami .play-prosess").click(function(a) {
			var c = $(this),
			d = c.parent().next(".sb-jplayer"),
			f = c.width(),
			c = c.offset().left;
